...
function renderTasks(tasks) {
  ...
  const toDo = tasks.map((task) => `<li>${task}</li>`);
}
...
function manageTasks(event) {
  ...
  // event.target will point to the actual icon clicked on. We need to get 
  //the parent li to work with however. HINT: Remember element.closest()? Look it up if you don't
...
// we need to attach listeners to the submit button and the list. Listen for a click, call the 'newTask' function on submit 
